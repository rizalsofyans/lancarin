var pixlr = function() {
    ...
    function buildUrl(opt) {
        ...
        for(var attribute in opt) {
        	if(attribute != 'service') url += ((url!='')?"&":"?") + attribute +"="+ escape(opt[attribute]);
    	}
    ...
    return {
		...
		window: function(options) {
        	var opt = extend(pixlr.settings, options || {});
			if(!window.open(buildUrl(opt), "pixlr", "location=0,status=0,scrollbars=0")) {
				alert("The editor window was blocked by your browser, please add pixlr.com to your pop-up blocker.");
			}
        },
