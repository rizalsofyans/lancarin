		  				...
		  				<div class="row">
		  					<div class="col-md-9 col-sm-6 mb-mobile-15">
		  						<div class="btn-group" role="group" aria-label="export">
									...
									<a href="javascript:;" class="btn btn-default check-payment" ><i class="fa fa-credit-card"></i> Check Payment</a>
								</div>
		  					</div>
		                    ...
		                    <tr>
		                    	...
		                    	<td>
		                        	<div class="btn-group btn-group-option <?=($key + 1 == count($result)?"dropup":"")?>">
									  	<a class="btn btn-default actionItem" href="<?=cn($module."/ajax_delete_item")?>" data-id="<?=$row->ids?>" data-redirect="<?=cn($module)?>" data-confirm="<?=lang("are_you_sure_want_delete_it")?>" class="actionItem"><i class="fa fa-trash-o"></i></a>
									</div>
		                        </td>
...
<script>
  $(function(){
    $('.check-payment').click(function(){
      var btn=$(this);
      $.ajax({
        url: '<?=site_url('payment/checkPayment')?>',
        dataType: 'json',
        beforeSend: function(){
        ...
        success: function(d){
          if(d.ok==1){
            alert('Found: '+ d.found +' new payment');
            if(d.found >0) location.reload();
          }else{
            alert(d.msg);
          }
          setTimeout(function(){
            btn.prop('disabled', false).removeClass('btn-warning').addClass('btn-default');
          },2000);
        },
        error: function(){
