		  				...
		  				<div class="row">
		  					<div class="col-md-9 col-sm-6 mb-mobile-15">
		  						<div class="btn-group" role="group" aria-label="export">
									...
									<a href="javascript:;" class="btn btn-default btn-recalculate"><i class="fa fa-refresh"></i> Recalculate</a>
								</div>
		  					</div>
		  			...
		  			<table class="table table-bordered table-datatable mb0" width="100%">
		                ...
		                ?>
		                    <tr>
		                        ...
		                        <?php $a=0;foreach ($columns as $column_name => $column_title){?>
		                        <td <?=$a==1? 'class="jml"':'';?>><?=custom_row($row->$column_name, $column_name, $module, $row->ids)?></td>
		                        <?php $a++;}?>
		                    </tr>
		                <?php }}?>
		                </tbody>
...
<div id="modal-jumlah" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">List proxy user</h4>
      </div>
...
<script>
  $(function(){
    $('.btn-recalculate').click(function(){
      var btn = $(this);
      $.ajax({
        url: '<?=site_url('proxies/autoAssignProxy');?>',
        beforeSend: function(){
	...
	$('.jml').click( function(){
      if($(this).text()>0){
        var proxy = $(this).prev().text().trim();
        $.ajax({
          url: '<?=site_url('proxies/getProxyCount');?>',
          data: {proxy: proxy, token: token},
          type: 'post',
          dataType: 'json',
          success: function(d){
            if(d.ok==1){
             $('#modal-jumlah').find('tbody').html(d.data); 
